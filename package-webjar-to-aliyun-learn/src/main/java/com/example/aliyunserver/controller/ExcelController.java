package com.example.aliyunserver.controller;


import com.alibaba.fastjson.JSON;
import com.example.aliyunserver.entity.AssetPointEntity_HI;
...
//                    return String.format("%.2f", cell.getNumericCellValue());
    ...
    public String importData(@ApiParam(value = "用户信息Excel导入数据", required = true) MultipartFile file) throws IOException {
        ...
        try {
            ...
            //assetPointServer.save(beans);
            //assetPointServer.save(beans.get(0));
            return JSON.toJSONString(beans.get(0));
        } catch (Exception e) {
            //throw  new ApplicationRuntimeException(e.getMessage());
            return "数据库操作失败";
        }
